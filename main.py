...
from src.data_store import autosave, clean_tokens, clear
from src.auth import register, login, logout
from src.error import PayloadError
from src.invoice import receive, update, delete, list
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        if sys.argv[1] == '--clear' and os.path.exists('saves/data.p'):
            os.remove('saves/data.p')
            clear_files('invoices_received/')
            clear_files('communication_report/')
