...
def set_data(updated):
    ...
    global DATA
    DATA = updated
    with open('saves/data.p', 'wb+') as new_save:
...
def autosave():
    ...
    try:
        with open('saves/data.p', 'rb') as old_save:
            old_data = pickle.load(old_save)
            set_data(old_data)
        print('/// Save file loaded successfully')
        # print(DATA)
    except FileNotFoundError:
        print('!!! No saved save file detected. Please ensure that any saves are stored in saves"')
