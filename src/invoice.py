...
from src.data_store import set_data, get_data
from src.json_report import create_json_report
...
def receive(token, invoice, output_format):
    ...
    if not os.path.exists('invoices_received'):
        os.mkdir('invoices_received')
    save_path = os.path.join('invoices_received', filename)
